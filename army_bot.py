...
@tree.command(name="accept", description="Принять нового бойца в ряды Army")
...
@tree.command(name="rank", description="Повышение или понижение бойца")
@app_commands.describe(
...
@tree.command(name="report", description="Отправить дневной или недельный отчёт")
@app_commands.describe(
...
@tree.command(name="storage", description="Отчёт по складу")
@app_commands.describe(
...
@tree.command(name="mass", description="Массовое взятие со склада — несколько предметов за раз")
@app_commands.describe(
